...
ZSHRC_TEMPLATE := $(CURDIR)/templates/zshrc.starter
...
.PHONY: help all core languages apps ai data cloud optional doctor \
	check-macos check-developer-tools zsh homebrew tap-hashicorp tap-mongodb tap-azure-functions tap-bun \
...
tap-bun: homebrew
	@echo "Ensuring Bun tap is available..."
...
setup_shell: homebrew link-oh-my-zsh-assets eza zoxide
	...
	elif grep -Fq '# Generated by mac-setup' "$(ZSHRC)"; then \
		echo "Refreshing legacy $(ZSHRC) starter config..."; \
		cp "$(ZSHRC_TEMPLATE)" "$(ZSHRC)"; \
	else \
...
bun: tap-bun
	...
	@brew list --formula bun >/dev/null 2>&1 || brew install oven-sh/bun/bun
